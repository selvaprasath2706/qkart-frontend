...
var { users, products, adventures } = require('./db');
...
const getAdventure = (adventureId) => {
    return new Promise((resolve, reject) => {
        adventures.findOne({ adventureId: adventureId }, (err, docs) => {
            if (err) {
                reject(err);
            }
            resolve(docs);
...
module.exports.getAdventure = getAdventure;

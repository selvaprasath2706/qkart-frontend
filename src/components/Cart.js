...
export const generateCartItemsFrom = (cartData, productsData) => {
  ...
  if (!cartData) return;
  // console.log(cartData.cartItems,productsData)
  cartData=cartData.cartItems
  const nextCart = cartData.map((item) => ({
    ...item,
    ...productsData.find((product) => item.product._id === product._id),
  }));
...
export const getTotalItems = (items = []) => {
  ...
  if (!items.length) return 0;
  const total = items.map((item) => item.quantity).reduce((total, n) => total + n);
  return total;
